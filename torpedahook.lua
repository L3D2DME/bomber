...
local L75 = { ["user"] = "dima t.", ["build"] = "ZАПЕЕЕЕН◣_◢" }
local L76 = L24.new_checkbox("AA", "Anti-aimbot angles", "torpedahook - " .. L22.lower(L75["user"]))
local L77 = nil;
local L78 = nil;
local L79 = nil;
local L80 = nil;
...
function watermark(L165, L166)
    ...
    local L167 = ""
    local L168 = "TORPEDAHOOK [" .. L22.upper(L75["build"]) .. "]" .. " | " .. L22.upper(L75["user"]) .. " | " .. L23.floor(L28.latency() * 1000) .. "MS"
    if L166 == "small" then
        L167 = "-"
        L168 = "TORPEDAHOOK   [" .. L22.upper(L75["build"]) .. "]" .. "   |   " .. L22.upper(L75["user"]) .. "   |   " .. L23.floor(L28.latency() * 1000) .. "MS"
    ...
    table.insert(L174, { text = "- ОТДЫХАЮ ВО ВРЕМЯ: " .. L22.upper(L70.current_condition), r = 240, g = 240, b = 240, a = 220 })
    table.insert(L174, { text = "- ЕБАШУ: " .. L22.upper(L28.current_threat() == nil and "?" or L22.sub(L27.get_player_name(L28.current_threat()), 0, 12)), r = 240, g = 240, b = 240, a = 220 })
    table.insert(L174, { text = "- ДТ АТЖАЛ: " .. (L3.get_double_tap() == false and "0" or "1"), r = 240, g = 240, b = 240, a = 220 })
    table.insert(L174, { text = "- %random%: " .. L22.upper(L23.abs(L70.current_desync)) .. "*", r = 240, g = 240, b = 240, a = 220 })
...
local L197 = function(L192, L193, L194, L195, L196)
    ...
    if L145(L192) and L132(L24.get(L90["visuals"][6]), "animations on scope") then
        wraith.desync_rect_dist = L136(L70.desync_rect_dist, 21 + 2, L26.frametime() * 15)
...
local L401 = function(L359, L360, L361, L362, L363)
    ...
    if L24.get(L360) == "minimal (og)" then
        ...
        if L24.get(L360) == "anti urine" then
            table.insert(L364, { text = "TORPEDA HOOK", r = 255, g = 165, b = 0, a = L70.pulse, size = L25.measure_text("b", "TORPEDA HOOK") / 2 })
            ...
            table.insert(L364, { text = "BETTER ONSHOT", r = 128, g = 230, b = 150, a = L24.get(L127.anti_aim.other.ref_onshotantiaim[1]) and L24.get(L127.anti_aim.other.ref_onshotantiaim[2]) and 255 or 100, size = L25.measure_text("b", "BETTER ONSHOT") / 2 })
        ...
        elseif L24.get(L360) == "recode alpha" then
            local L389 = ("TORPEDAHOOK\a%s RECODE"):format(L22.format("%02X%02X%02X%02X", L365, L366, L367, L23.max(L368, 100)))
            local L390 = "DT"
            local L391 = "ON SHOT"
...
local L512 = function(L507)
    ...
    if L132(L24.get(L90["misc"][2]), "fire") then
        ...
        g_notify:add(L22.format("уебал по %s's %s на %i дамага (%d%%) bt=%i (%ims) body=%iº", L27.get_player_name(L507.target), L508, L507.damage, L507.hit_chance, L510, L15(L510 * 1000), L511), { 210, 210, 255, 255 }, 5, nil, "fire")
    end;
...
local L518 = function(L513)
    ...
    elseif L517 == 0 and L516 == L514 then
        if L132(L24.get(L90["misc"][2]), "hurt self") then
            g_notify:add(L22.format("полоснул вену себе в %s на %d дамага (%d жить осталось)", L515, L513.dmg_health, L513.health), { 255, 255, 255, 255 }, 5)
        end
    elseif L517 ~= 0 and L516 == L514 then
        if L132(L24.get(L90["misc"][2]), "hurt") then
            g_notify:add(L22.format("нам    зарядил по кипятильнику %s в %s на %d дамага (%d жить осталось)", L27.get_player_name(L517), L515, L513.dmg_health, L513.health), { 255, 255, 255, 255 }, 5)
        end
    end
end;
local L520 = function(L519)
    ...
    g_notify:add(L22.format('абасрался из за %s', L519.reason), { 255, 255, 255, 255 }, 5)
end;
